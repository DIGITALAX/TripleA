...
impl AgentManager {
    pub fn new(agent: &TripleAAgent) -> Option<Self> {
        ...
        match contracts {
            ...
            None => {
                eprintln!("Failed to initialize contracts for agent with ID: {}", agent.id);
                None
            }
        }
    }
    ...
    pub async fn resolve_activity(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        match collections_info {
            Ok(info) => {
                ...
                if info.len() < 1 {
                    println!("No collections for agent this round for agent_{}", self.agent.id);
                    return Ok(());
                }
                                    ...
                                    match result {
                                        Ok(balance) => {
                                            ...
                                            if balance <= U256::from(100_000_000_000_000_000u128) {
                                                let method = self.access_controls_contract.method::<
                                                    _,
                                                    H256
                                                >("faucet", (
                                                ...
                                                match method {
                                                    Ok(call) => {
                                                                ...
                                                                tx.as_eip1559_ref()
                                                        {
                                                                ...
                                                                ).await
                                                            {
                                                                ...
                                                                Err(e) => {
                                                                    eprintln!(
                                                                        "Error sending the transaction to claim GRASS: {:?}",
                                                                        e
                                                                    );
                                                                    Err(Box::new(e))?
                                                                }
                                                    ...
                                                    Err(err) => {
                                                        self.current_queue = Vec::new();
                                                        eprintln!(
                                                            "Error in method for Faucet claim: {:?}",
                                                            err
                                                        );
    ...
    async fn pay_rent(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        for collection in &self.current_queue {
            ...
            for price in &collection.collection.prices {
                ...
                let method = self.agents_contract.method::<_, U256>("getAgentRentBalance", (
        ...
        println!("Rent: {:?} {:?}\n", rent_collection_ids.clone(), rent_tokens);
        ...
        if rent_collection_ids.len() > 0 {
            ...
            let method = self.agents_contract.method::<(Vec<Address>, Vec<U256>, U256), H256>(
                "payRent",
                (rent_tokens.clone(), rent_collection_ids.clone(), U256::from(self.agent.id as u64))
            );
                ...
                Ok(call) => {
                    ...
                    if let Some(tx_request) = tx.as_eip1559_ref() {
                        ...
                        self.current_queue.retain(|item|
                            rent_collection_ids.contains(&item.collection_id)
                        );
                        ...
                        Err(
                            Box::new(
                                io::Error::new(io::ErrorKind::Other, "Error in sending Transaction")
                            )
                        )
                    }
                }
            ...
            println!("No collection Ids with sufficient tokens for agent_{}", self.agent.id);

            self.current_queue.retain(|item| {
                ARTISTS.iter().any(|a| a.eq_ignore_ascii_case(&item.collection.artist))
            });
    ...
    async fn get_collections_info(
        ...
        match response {
                    ...
                    Ok(result) => {
                        ...
                        for agent_created in agent_createds {
                            ...
                            for worker in agent_created["workers"].as_array().unwrap_or(&vec![]) {
                                let collection_id = U256::from_dec_str(
                                    worker["collectionId"].as_str().unwrap_or("0")
                                ).unwrap_or_default();
                                ...
                                let username = handle_lens_account(
                                    &artist,
                                    true
            ...
            Err(err) => {
                ...
                Err(
                    Box::new(io::Error::new(io::ErrorKind::TimedOut, format!("Timeout: {:?}", err)))
                )
            }
        }
    }
    ...
    async fn queue_lens_activity(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let adjusted_remaining_time = if remaining_time > 0 { remaining_time } else { 7200 };

        let queue = self.current_queue.clone();
        let queue_size = queue.len() as i64;

        let interval = if queue_size > 0 { adjusted_remaining_time / queue_size } else { 0 };
        ...
        for activity in queue {
            ...
            match tokens {
                Ok(new_tokens) => {
                    ...
                    self.current_queue.retain(|item| item.collection_id == activity.collection_id);
...
async fn cycle_activity(
    ...
    let total_activities =
        activity.worker.lead_frequency.as_u64() +
    ...
    let handles: Vec<_> = tasks
        ...
        .map(|(i, task)| {
            ...
            tokio::spawn(async move {
                tokio::time::sleep(
                    std::time::Duration::from_secs(((i as i64) * activity_interval) as u64)
                ).await;
    ...
    println!("Finished cycle_activity for Agent_{} and Activity {:?}", agent.id, activity);
}
