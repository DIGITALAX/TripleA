        ...
        if (entityAgent) {
          ...
          if (!tryActive.reverted) {
            ...
            for (let k = 0; k < active.length; k++) {
              cols.push(Bytes.fromByteArray(ByteArray.fromBigInt(active[k])));
            }
            entityAgent.activeCollectionIds = cols;
          }
    ...
    if (entityDrop.collections) {
      for (let i = 0; i < (entityDrop.collections as Bytes[]).length; i++) {
        ...
        if (entityCollection) {
          ...
          if (agents) {
            for (let k = 0; k < (agents as BigInt[]).length; k++) {
              let entityAgent = AgentCreated.load(
                Bytes.fromByteArray(
                  ByteArray.fromBigInt((agents as BigInt[])[k])
                )
              );
          ...
          for (
            let k = 0;
            k < (entityCollection.prices as Bytes[]).length;
            k++
          ) {
            let price = Price.load((entityCollection.prices as Bytes[])[k]);
